...
def _es_cacheable(t):
    low=(t or "").lower()
    return not (t.startswith("⚠️") or t.startswith("📅") or t.startswith("Según la información oficial") or len(t)<60 or "ayudarte hoy" in low or "no está en el contexto" in low or "===" in t or "documento " in low or len(t)>900)
...
async def report(req:Request):
    ...
    return {"total":len(lines),"hoy":sum(1 for l in lines if l.get("ts","").startswith(hoy)),"top":c.most_common(10),"idiomas":dict(idi)}
@app.get("/api/topfaq")
async def topfaq():
    now=time.time()
    try:
...
<span class="etiq">3️ Archivo</span><div id="drop"> Arrastra o toca</div><input id="ffile" type="file" style="display:none">
...
let t=' Total: '+d.total+' · Hoy: '+d.hoy+' · Idiomas: '+JSON.stringify(d.idiomas)+'\\n\\n Más frecuentes:\\n'+d.top.map((x,i)=>(i+1)+'. '+x[0]+' ('+x[1]+')').join('\\n');
